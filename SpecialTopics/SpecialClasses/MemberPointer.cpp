...
int main()
{
    ...
    while (fQuit == false)
    {
        ...
        // the pointer accesses the object and pFunc accesses the function
        (ptr->*pFunc)();
        // no reason to call delete on pFunc because this is a pointer to code, not to an object on the heap (would result in a compile time error)
        delete ptr;
    }
    return 0;
}